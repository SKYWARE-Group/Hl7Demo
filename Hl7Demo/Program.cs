    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            foreach (string file in files)
            {
                ...
                Console.WriteLine($"   Given name: {oml.PATIENT.PID.GetPatientName(0)?.GivenName?.Value}"); // Mandatory
                ...
                {
                    // At least LAB^PI must be provided
                ...
                // Ordered items
                Console.WriteLine($"Ordered examinations:");
                foreach(OML_O21_ORDER orderGroup in oml.ORDERs)
                {
                    ...
                    // Optional, only in cases where ordering physician is known
                    if (orderGroup.ORC.OrderingProviderRepetitionsUsed > 0)
                    {
